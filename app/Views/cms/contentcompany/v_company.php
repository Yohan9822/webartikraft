...
<div class="main-content content margin-t-4">
    <div class="card w-100 rounded shadow-sm p-x" style="position: relative;">
        ...
        <div class="card-body">
            <form action="<?= !empty($row) ? getURL('cms/contentcompany/update') : getURL('cms/contentcompany/add') ?>" id="form-company" method="post" style="padding-inline: 0px;">
                <input type="hidden" id="id" name="id" value="<?= !empty($row) ? encrypting($row->id) : '' ?>">
                <div class="form-group">
                    <textarea name="content_company" id="content_company" class="form-input" placeholder="Enter Company content"><?= !empty($row) ? $row->content : '' ?></textarea>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        CKEDITOR.replace('content_company', {
            ...
            filebrowserUploadUrl: '<?= getURL("cms/contentcompany/upload") ?>',
            filebrowserBrowseUrl: '<?= getURL("cms/contentcompany/browse") ?>'
        });
        ...
        elements.form.formSubmit({
            beforeSubmit: () => {
                ...
                const contentData = CKEDITOR.instances.content_company.getData();
                console.log('CKEditor Data:', contentData);

                elements.form.find('[name="content_company"]').val(contentData);
                return true;
            },
            successCallback: (res) => {
